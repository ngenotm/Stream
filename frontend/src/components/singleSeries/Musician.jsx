...
const Musician = ({ custom, musician }) => (
    <div>
        <p className={`text-c-grey-60 ${custom && "text-super-base"}`}>Musician</p>
        <div
        ...
        >
            ...
            <div className="flex flex-col justify-around capitalize">
                {/* <Link href={`/musicians/${id}`} > */}
                    <h5 className={`text-white ${!custom && "text-super-sm"}  tracking-wide`}>{musician.name}</h5>
                    <span className={`block text-c-grey-60 ${custom ? "text-super-sm" : "text-sm"} `}>
